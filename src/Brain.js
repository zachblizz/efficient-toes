...
function checkDiagRowAndCol(row, char, winner, board) {
  let rowCount = 0;
  let colCount = 0;

  for (let i = 0; i < board.length; i++) {
    if (board[i][row] !== EMPTY && board[i][row] !== char) {
      winner.winner = false;
    } else if (board[i][row] !== EMPTY) {
      rowCount++;
      if (rowCount === 3) {
        winner.winner = true;
        break;
...
export const Brain = {
  ...
  diagonalWinner(char, board, row) {
    ...
    if (winCount !== 3) {
      winCount = 0;
      for (let i = board.length-1, j = 0; i >= 0; i--) {
        if (board[i][j++] !== char) {
          this.winner.winner = false;
          break;
    ...
    if (winCount !== 3) { 
      checkDiagRowAndCol(row, char, this.winner, board);
    } else {
      this.winner.winner = true;
    }
